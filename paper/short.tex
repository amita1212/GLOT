...
grid, \textbf{Stage C is the strongest}: $+1.42$ MCC on CoLA with $13/15$ seeds
positive, significant on both a paired $t$-test and an exact sign test, and
A${+}$C is the best arm on STS-B. \textbf{Stage B is significantly harmful} on
both an encoder and a decoder backbone, surviving Bonferroni. ``Hyperbolic
...
\item \textbf{A stage-wise hyperbolic extension} (\S\ref{sec:arms}). Under an
      ...
      best arm on STS-B. The hyperbolic readout (Stage B) is significantly
